...
Konfiguracja={
    ...
    "Klucz_zabezpieczający_scenariusz":"**823$%@@7&*&*!",
    "Klucz_zabezpieczający_symulacje":"SyMuLaCjA112!",
    "szer_kom":6
    }
...
Slownik_opisów={
    0: """
        ...
        zależy tylko i wyłącznie od liczby jej żywych sąsiadów. A gdy „żywa” komórka dojdzie do 
        krawędzi planszy, umiera.
        ...
        2)	Poruszanie po planszy: Z pomocą lewego przycisku myszy klikasz/przeciągasz
        ...
        - komórki z niektórych zestawów mogą szybko umrzeć, bądź formować się w te same pozycje w 
        nieskończonosć 
        - dane zestawy posiadają swoje własne nazwy, pochodzące z specjalnej biblioteki, dostępnej w 
        internecie, na przykład na stronie https://conwaylife.appspot.com/library
        
        ***************************************************************************************
        Zestawy zapisują się w formacie txt, plik z zestawem posiada kod zabezpieczajacy oraz
        aktualne pozycje żywych komórek zapisanych w formie pary liczb, odpowiadających numerze
        kolumny i wiersza danej żywej komórki.
        
        W przypadku zestawów "stwórz ze zdjęcia", są to pliki graficzne, zapisane w formacie png.
        ***************************************************************************************
        ...
        ***************************************************************************************
        Parametry zapisują się w formacie txt, plik składa się z 5 linijek:
        ...
        ***************************************************************************************
        Scenariusze zapisują się w formacie txt, plik składa się z kilkunastu linijek:
        - klucz zabezpieczający
        - wartosc aktualna: samotnosc
        - wartosc aktualna: scisk
        - wartosc aktualna: nadzieja
        - wartosc aktualna: ile_generacji
        - informacja o zakończeniu parametrów: __PARAMETRY_KONIEC__
        - aktualne pozycje żywych komórek zapisanych w formie pary liczb, odpowiadających numerze
        kolumny i wiersza danej żywej komórki
        *************************************************************************************** 
        ...
        który odpowiada za pobranie pozycji komórek żywych i zapisanie ich w takiej formie 
        aby mogła zostać stworzona Animacja.
        
        Po kliknięciu „Symulacja” musisz się uzbroić w cierpliwość i poczekać kilka sekund, 
        ...
        (UWAGA: jeżeli otworzył się folder a nie widzisz pliku, powinnien on 
        znajdować się tam gdzie została pobrana aplikacja), 
        po wybraniu pliku animacja zaczyna zostają pracę, pod odczekaniu kolejnych kilku
        sekund na nowym oknie pojawia się animacja aktualnego zestawu. 
...
class Okno_interfejsu_glownego(wx.Frame):
    ...
    def wx_test_do_symulacji(self,evt):
        if len(aktualny_zestaw)!=0:
            self.wx_symulacja()
            self.wx_wyświetl_wiadomość(("Ukończono symulacje"),"info")
        else:
    ...
    def wx_animacja(self,nazwa_pliku):
        ...
        with open(nazwa_pliku) as f: # jest tylko 1  otawrcie pliku zmiast każdrazowego przy wywołaniu funkcji
            ...
            if klucz_zab!=Konfiguracja['Klucz_zabezpieczający_symulacje']:
                self.wx_wyświetl_wiadomość(("Wybrany plik nie jest plikiem symulacji"),"error")
                self.wx_test_uaktualnij_pasek(1, "")
                return 
            while True:
    ...
    def wx_symulacja(self):
        ...
        while licznik < ile_gen:
            ...
            if wynikowa in wszystkie_gen:
                ...
                return
            # Trzeba dodać warunek, jeli plansza jest pusta to też zatrzymac symulacje
        ...
        if scisk.isnumeric() and samotnosc.isnumeric() and nadzieja.isnumeric()  and generacje.isnumeric():
            ...
            if samotnosc >=0 and scisk >0 and generacje >0 and nadzieja >0:
                if samotnosc <=8 and scisk <=8 and nadzieja<=8:
                    ...
                    Okno_interfejsu_glownego.wx_wyświetl_wiadomość(self, "Zmieniono parametry", "info")
                else:
            ...
            Okno_interfejsu_glownego.wx_wyświetl_wiadomość(self,"Wprowadzone wartosci nie sa liczbami","error")
...
class Okno_pomocy(wx.Frame):
   ...
   def InitUI(self):   
       ...
       panelA = wx.lib.scrolledpanel.ScrolledPanel(self.scroll, -1, style=wx.SIMPLE_BORDER, size=(800,1950)) # to 2 służy do regulacji
       #jak daleko w doł możesz zeskrolować
       panelA.SetupScrolling()
       static_text=wx.StaticText(panelA, -1, Slownik_opisów[0])
...
def wczytaj_scenariusz(nazwa_pliku):
    ...
    if os.path.isfile(nazwa_pliku) :
        ...
        with open(nazwa_pliku,"r") as plik:
            ...
            licz = 0
            a=a.replace('\n', '')
            if a == Konfiguracja["Klucz_zabezpieczający_scenariusz"]:
                while True:
                    licz += 1
                    a=plik.readline()
...
def zapisz_scenariusz(nazwa_pliku):
    ...
    if os.path.isfile(nazwa_pliku) :
        ...
        with open(nazwa_pliku,"w") as f:
            f.write(Konfiguracja["Klucz_zabezpieczający_scenariusz"]+"\n")
            for k,v in Parametry.items():
                linia=k+":"+str(v)+"\n"
                f.write(linia)
...
def zapisz_generacje(symulacja, nazwa_pliku):
    ...
    f.write(Konfiguracja["Klucz_zabezpieczający_symulacje"]+"\n")
...
def z_gifa(zbiór_bitmap):
    ...
    """
    imageio.mimsave(os.getcwd()+'\\movie_CA.gif', zbiór_bitmap)
